...
class CircleDrawer(Frame):
    ...
    def init_ui(self):
        ...
        alg_label = Label(self, text='Выберите алгоритм для построения первой окружности:')
        ...
        end_label = Label(self, text='Радиус:')
        end_label.grid(row=5, column=1)
        r_label = Label(self, text='R:')
        r_label.grid(row=6, column=0)
        self.r_entry = Entry(self)
        self.r_entry.grid(row=6, column=1)
        self.r = 0
        ...
        alg_sec_label = Label(self, text='Выберите алгоритм для построения второй окружности:')
    ...
    def draw_sec_circle(self):
        ...
        if (self.r == 0):
            box.showerror("Ошибка", "Сначала необходимо построить главную окружность")
            return
        self.canvas.scale("all", 0, 0, 0.5, 0.5)

        if (algorithm == 'Канонический'):
            circle_canon(self.canvas, self.color_second, self.x1, self.y1, self.r)
        elif (algorithm == 'Параметрический'):
            circle_param(self.canvas, self.color_second, self.x1, self.y1, self.r)
        elif (algorithm == 'Брезенхем'):
            circle_brez(self.canvas, self.color_second, self.x1, self.y1, self.r)
        elif (algorithm == 'Средняя точка'):
            circle_middle(self.canvas, self.color_second, self.x1, self.y1, self.r)
        else:
            self.canvas.create_oval(self.x1 - self.r, self.y1 - self.r, self.x1 + self.r, self.y1 + self.r, outline=self.color_second)
...
целыми числами""")
        ...
        if (algorithm == 'Канонический'):
            circle_canon(self.canvas, self.color_first, self.x1, self.y1, self.r)
        elif (algorithm == 'Параметрический'):
            circle_param(self.canvas, self.color_first, self.x1, self.y1, self.r)
        elif (algorithm == 'Брезенхем'):
            circle_brez(self.canvas, self.color_first, self.x1, self.y1, self.r)
        elif (algorithm == 'Средняя точка'):
            circle_middle(self.canvas, self.color_first, self.x1, self.y1, self.r)
        else:
            self.canvas.create_oval(self.x1 - self.r, self.y1 - self.r, self.x1 + self.r, self.y1 + self.r, outline=self.color_first)
